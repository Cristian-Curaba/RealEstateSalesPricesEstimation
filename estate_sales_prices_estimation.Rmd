...
write.csv(Constructions, "Constructions.csv" )
```
...
to avoid collinearity and over complexity.

As baseline model, we are going to consider a linear model with
predictor variable: "price at beginning".
...
Since there's overdispersion and high residuals let's try the negative
binomial model with link function "log" and "identity":
...
Confronting AIC and Residual we can easily choose the negative binomial
model with link function "identity". We also tried other standard
...
```{r}
plot(as.factor(BuildingZIPCode), SalesPrices, xlab="Building ZIP codes", ylab="Sales Prices")
```

May be a good idea to merge some of the ZIP codes that doesn't influence
